...
function cleanupFirebaseApp(app) {
    if (app && typeof app.delete === 'function') {
        try {
            ...
            console.log('Note lors du nettoyage Firebase:', e.message);
            return false;
        }
    }
    return false;
...
// Authentification SIMPLIFIÉE - sans vérification de temps dans Firebase
async function authenticateWithCode(code) {
    ...
    try {
        ...
        // 3. Générer le challenge et la réponse
        currentChallengeId = generateId();
        const challenge = generateId();
        const timestamp = Date.now();
        
        // 4. Calculer et sauvegarder la réponse (SANS timestamp de vérification)
        const response = await sha256(code + challenge);
        currentResponseHash = await sha256(response);
        
        // Sauvegarder dans Firebase (sans vérification de temps)
        await tempDb.ref(`valid_responses/${currentResponseHash}`).set({
            challengeId: currentChallengeId,
            response: response
        });
        
        // 5. Stocker le token localement avec timestamp pour 30 minutes
        ...
        if (error.message.includes('incorrect')) {
            ...
            showAuthError('Erreur de configuration');
        } else {
...
async function cleanupAuthData() {
    ...
    try {
        // Nettoyer la réponse dans Firebase
        ...
        if (savedResponseHash && savedResponseHash !== currentResponseHash) {
            await database.ref(`valid_responses/${savedResponseHash}`).remove();
        }
        
    } catch (error) {
        console.log('Note: Données d\'auth déjà nettoyées ou non accessibles');
    }
}
...
// Vérifier l'authentification au chargement (30 minutes côté client seulement)
async function checkExistingAuth() {
    ...
    // Vérifier si la session est encore valide (30 minutes) - COTÉ CLIENT SEULEMENT
    ...
    try {
        ...
        // Tester la connexion - SIMPLE vérification d'accès
        ...
        // En cas d'erreur PERMISSION_DENIED, c'est que le token n'est plus valide dans Firebase
...
// Gérer le timer de session (30 minutes côté client)
